...
function parseHTMLRichText(
	...
	const parseNode = (node: HTMLElement): Descendant => {
		...
		const children = Array.from(node.children).map(child => parseNode(child as HTMLElement));
		const tagName = node.tagName.toLowerCase();

		// Функция для извлечения общих свойств
		const extractCommonProps = () => ({
			horisontalAlignment: (node.style.textAlign || 'left') as HorisontalAlignment,
		...
		switch (tagName) {
			...
			case 'h6': {
				const headingType = `heading_${
					['one', 'two', 'three', 'four', 'five', 'six'][parseInt(tagName[1]) - 1]
				}` as const;
				return {
					type: headingType,
					...extractCommonProps(),
			...
			case 'p':
				return {
					...
					...extractCommonProps(),
					lineHeight:
						parseFloat(node.style.lineHeight) || conf.DEFAULT_TEXT_STYLES.lineHeight,
					children,
				};
	...
	const data: RichTextData & { id: string } = {
		...
		verticalAlignment:
			(el.getAttribute('data-vertical-alignment') as VerticalAlignment) || 'top',
		children,
	};
...
function parseHTMLFrame(el: HTMLElement): {
	...
	const childrenMap = Array.from(el.children)
		.filter(child => child.id !== `${el.id}_text` && !child.classList.contains('link-object'))
		.map(child => positionAbsolutely(child as HTMLElement, el))
		.reduce((acc: { [id: string]: ItemData }, child) => {
			acc[child.id] = parsersHTML[child.tagName.toLowerCase()](child as HTMLElement);
			return acc;
		}, {});
...
function parseHTMLConnector(el: HTMLElement): ConnectorData & { id: string } {
	...
	const startRelativeX = parseFloat(el.getAttribute('data-start-point-relative-x') || '');
	const startRelativeY = parseFloat(el.getAttribute('data-start-point-relative-y') || '');
	const startTangent = parseFloat(el.getAttribute('data-start-point-tangent') || '');
	const startSegment = parseFloat(el.getAttribute('data-start-point-segment') || '');
	...
	const endRelativeX = parseFloat(el.getAttribute('data-end-point-relative-x') || '');
	const endRelativeY = parseFloat(el.getAttribute('data-end-point-relative-y') || '');
	const endTangent = parseFloat(el.getAttribute('data-end-point-tangent') || '');
	const endSegment = parseFloat(el.getAttribute('data-end-point-segment') || '');
	...
	const connectorData: ConnectorData & { id: string } = {
		...
		endPoint,
		startPointerStyle:
			(el.getAttribute('data-start-pointer-style') as ConnectorPointerStyle) || 'None',
		endPointerStyle:
			(el.getAttribute('data-end-pointer-style') as ConnectorPointerStyle) || 'None',
		lineStyle: (el.getAttribute('data-line-style') as ConnectorLineStyle) || 'straight',
		lineColor: el.getAttribute('data-line-color') || '',
		lineWidth: parseInt(el.getAttribute('data-line-width') || '1') as ConnectionLineWidth,
		borderStyle: (el.getAttribute('data-border-style') as BorderStyle) || '',
		text: new DefaultRichTextData(),
		linkTo: el.getAttribute('data-link-to') || undefined,
	};
