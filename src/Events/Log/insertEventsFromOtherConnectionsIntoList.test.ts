...
import { ItemData, Mbr, RichText, RichTextOperation } from "Items";
...
export function getAddItemOp(
  board: Board,
  { itemId = "shapeId", dataOverrides = {}, timeStamp }: GetAddItemOpParams = {}
): CreateItem {
...
describe("insertEventsFromOtherConnectionsIntoList", () => {
  ...
  test("local unconfirmed: insert text and update selection. After reverting and applying local selection should remain the same", () => {
    ...
    const remoteEvents = Array.from({ length: remoteEventsAmount }, (_, i) => {
    ...
    insertEventsFromOtherConnectionsIntoList(remoteEvents, eventsList, board);
