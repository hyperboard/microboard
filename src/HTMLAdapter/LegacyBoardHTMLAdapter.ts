...
export function serializeBoardToLegacyHTML(board: Board): string {
...
export function deserializeLegacyHTMLAndEmitToBoard(board: Board, stringedHTML: string): string[] {
  ...
  if (items) {
    ...
    const data = Array.from(items.children).map((el) =>
      parseLegacyHTML(el as HTMLElement)
    );
...
export function deserializeLegacyHTMLToBoard(board: Board, stringedHTML: string): void {
  const parser = conf.getDOMParser();
  const doc = parser.parseFromString(stringedHTML, "text/html");
