...
export const createPlaceholderImage = (
  ...
  const videoIcon = documentFactory.createElement("img") as HTMLImageElement;
  videoIcon.src = VIDEO_ICON_SRC;

  const canvas = documentFactory.createElement(
    "canvas"
...
export class VideoHTMLRenderer implements IItemHTMLRenderer {
  render(item: BaseItem, documentFactory: DocumentFactory): HTMLElement {
    ...
    const { translateX, translateY, scaleX, scaleY } =
      videoItem.transformation.getMatrixData();
    const transform = `translate(${translateX}px, ${translateY}px) scale(${scaleX}, ${scaleY})`;

    const previewUrl = videoItem.getPreviewUrl();
    div.style.backgroundImage = previewUrl
      ? `url(${previewUrl})`
      : `url(${
          createPlaceholderImage(
            documentFactory,
            videoItem.videoDimension.width,
            videoItem.videoDimension.height
          ).src
        })`;

    div.id = videoItem.getId();
    div.style.width = `${videoItem.videoDimension.width}px`;
    div.style.height = `${videoItem.videoDimension.height}px`;
    div.style.transformOrigin = "top left";
    div.style.transform = transform;
    div.style.position = "absolute";
    div.style.backgroundSize = "cover";
    div.setAttribute("video-url", videoItem.getUrl());
    div.setAttribute("preview-url", videoItem.getPreviewUrl());
    div.setAttribute("extension", (videoItem as any).extension);
    div.setAttribute("is-storage-url", (videoItem as any).isStorageUrl ? "1" : "");
    div.setAttribute("data-link-to", videoItem.linkTo.serialize() || "");
    if (videoItem.getLinkTo()) {
      const linkElement = renderLinkToHTML(videoItem.linkTo, documentFactory);
...
registerHTMLRenderer("Video", new VideoHTMLRenderer());
