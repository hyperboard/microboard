...
export const createPlaceholderImage = (width: number, height: number): HTMLImageElement => {
...
export class VideoItem extends Mbr {
	...
	applyVideoData({ previewUrl = '', url = '' }: { previewUrl?: string; url?: string }): void {
		this.previewUrl = previewUrl;
		this.setPreview(this.preview, previewUrl);
		this.setUrl(url);
	}
	...
	renderHTML(documentFactory: DocumentFactory): HTMLElement {
		...
		div.style.backgroundImage = this.previewUrl
			? `url(${this.previewUrl})`
			: `url(${
					createPlaceholderImage(this.videoDimension.width, this.videoDimension.height)
						.src
			  })`;
