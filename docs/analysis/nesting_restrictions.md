...
- **Current Implementation**: `Group` and `Frame` calculate their `getMbr()` by unioning children's local bounds and transforming them into **absolute world coordinates**.
- **Conflict**: The nesting logic in `BaseItem` expects children to store **local coordinates** relative to their parent.
...
- **Current Engine**: The `Board` and `BoardSelection` logic are optimized for a two-tier hierarchy (Board -> Container -> Item). 
